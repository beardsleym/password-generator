            ...
            <div className="relative mb-20">
              <input type="text"  value={password} onChange={(event) => handleInput(event.target.value)} className="absolute tracking-wider border rounded w-full py-4 px-2 sm:px-5 text-gray-700 border-gray-300 focus:outline-none font-mono sm:text-xl" />
                ...
                <button onClick={handleClick} className="bg-white absolute top-2 right-2 p-2" type="button">
                  ...
                  {/* WARNING MESSAGE */}
                  {feedback?.feedback?.warning && isPwned > 0 && <p className="absolute top-11 left-1.5 font-light text-xs text-red-600 px-4"><span className="hidden sm:inline">{feedback.feedback.warning}.</span> <span className="font-semibold">{isPwned.toLocaleString()} data {isPwned > 1 ? 'breaches' : 'breach'}.</span> </p>} 
          ...
          <div className="flex items-center justify-center w-full mt-4">
            ...
            > 
              ...
              <div className="relative">
                {/* <!-- input --> */}
                <input id="toogleA" type="checkbox" className="sr-only" checked={passphrase} onChange={(event) => handlePassphrase(event.target.checked)}/>
                {/* <!-- line --> */}
